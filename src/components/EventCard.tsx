...
export default function EventCard({ event, showActions = true }: EventCardProps) {
  const { savedIds, toggleSave, registeredIds, getRegisteredCount } = useApp();
  ...
  return (
    <div className="group bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
      {/* Tilted Image Header */}
      <div className="relative h-48 overflow-hidden bg-gradient-to-br from-[#112E81]/10 to-[#5790F4]/20">
        <TiltedCard
            ...
            <div className="relative w-full h-full p-3 flex flex-col justify-between pointer-events-none">
              ...
              <div className="flex items-center justify-between pointer-events-auto">
                ...
                {showActions && (
                  <button
                    ...
                    className={cn(
                      "w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 backdrop-blur-md shadow-md border",
                      isSaved
                        ? "bg-rose-100 text-rose-600 border-rose-200 shadow-lg"
                        : "bg-white/90 text-[#112E81] border-[#112E81]/20 hover:bg-white hover:text-rose-500"
                    )}
              ...
              {isRegistered && (
                <div className="pointer-events-auto">
                  <span className="inline-block px-2.5 py-0.5 bg-[#112E81] text-[#5790F4] border border-[#5790F4]/40 text-xs font-bold rounded-full shadow-lg">
                    ✓ Registered
                  </span>
                </div>
              )}
      ...
      <div className="p-5">
        ...
        <div className="space-y-1.5 mb-4">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-500">
            <MapPin className="w-3.5 h-3.5 text-[#5790F4] flex-shrink-0" />
          ...
          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={cn(
                ...
                progress >= 80 ? "bg-amber-500" : "bg-gradient-to-r from-[#112E81] to-[#5790F4]"
              )}
