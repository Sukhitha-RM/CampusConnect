...
import { CalendarDays, CheckCircle2, Heart, Bell, ArrowRight } from "lucide-react";
...
export default function Dashboard() {
  ...
  const featuredEvents = useMemo(() => events.filter((e) => e.featured).slice(0, 4), []);
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Link to="/events">
          <button className="hidden md:flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#112E81] text-white text-sm font-semibold hover:bg-[#0e2468] hover:-translate-y-0.5 transition-all shadow-sm shadow-[#112E81]/30 border border-[#5790F4]/20">
            Explore Events <ArrowRight className="w-4 h-4 text-[#5790F4]" />
          </button>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: CalendarDays, label: "Events This Month", value: monthlyCount, color: "text-[#5790F4]", bg: "bg-[#5790F4]/15" },
      ...
      {/* Hero banner */}
      <div className="relative bg-gradient-to-br from-[#112E81] via-[#0e2468] to-[#070b1e] rounded-3xl p-8 text-white overflow-hidden border border-[#5790F4]/20 shadow-xl">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_right,rgba(87,144,244,0.15),transparent)] pointer-events-none" />
        <div className="relative z-10">
          <p className="text-[#5790F4] text-sm font-bold mb-2 uppercase tracking-wider">Discover Campus Life</p>
          ...
          <Link to="/events">
            <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#5790F4] text-[#070b1e] text-sm font-extrabold hover:bg-[#3B82F6] transition-colors shadow-lg shadow-[#5790F4]/20">
              Explore Events <ArrowRight className="w-4 h-4" />
            </button>
          </Link>
        </div>
        {/* Decorative elements */}
        <div className="absolute right-8 top-1/2 -translate-y-1/2 w-36 h-36 bg-[#5790F4]/10 rounded-full blur-xl pointer-events-none" />
      </div>
      ...
      <div>
        <div className="flex items-end justify-between mb-5">
          ...
          <Link to="/my-registrations" className="text-sm font-semibold text-[#112E81] hover:text-[#5790F4] flex items-center gap-1">
        ...
        {registeredEvents.length === 0 ? (
          <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-10 text-center">
            ...
            <Link to="/events" className="text-[#112E81] text-sm font-bold hover:text-[#5790F4] mt-2 inline-block">
              Browse Events →
            </Link>
          </div>
        ) : (
            ...
            {registeredEvents.slice(0, 3).map((event) => {
              return (
                <div key={event.id} className="group bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-lg transition-all">
                  <div className="relative h-40 overflow-hidden bg-gradient-to-br from-[#112E81]/10 to-[#5790F4]/20">
                    <img src={event.image} alt={event.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                      onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
                    <span className="absolute top-3 left-3 px-2.5 py-0.5 bg-[#112E81] text-[#5790F4] text-[10px] font-bold rounded-full border border-[#5790F4]/30">
                      {event.category}
                    </span>
                  </div>
                  <div className="p-4">
                    ...
                    <Link to={`/events/${event.id}`} className="text-xs font-bold text-[#112E81] hover:text-[#5790F4] flex items-center gap-1">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        <div className="flex items-end justify-between mb-5">
          <div>
            ...
            <h2 className="text-lg font-black text-slate-900">Featured Events</h2>
          </div>
          <Link to="/events" className="text-sm font-semibold text-[#112E81] hover:text-[#5790F4] flex items-center gap-1">
            View all <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
          ...
          {featuredEvents.map((event) => {
            ...
            return (
              <div key={event.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors group">
                ...
                <Link to={`/events/${event.id}`} className="text-xs font-bold text-[#112E81] hover:text-[#5790F4] flex items-center gap-1 flex-shrink-0">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        <div className="flex items-end justify-between mb-5">
          ...
          <Link to="/notifications" className="text-sm font-semibold text-[#112E81] hover:text-[#5790F4] flex items-center gap-1">
            View all <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
        <div className="space-y-3">
